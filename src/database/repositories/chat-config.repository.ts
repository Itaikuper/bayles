...
export class ChatConfigRepository {
  ...
  getByJid(jid: string): ChatConfig | null {
    const row = this.db
      ...
      .get(jid) as ChatConfig | undefined;
    return row ?? null;
  }
  ...
  create(config: CreateChatConfig): ChatConfig {
    this.db
      ...
      .run(
        ...
        config.auto_reply_message ?? null,
  ...
  isWithinSchedule(jid: string): boolean {
    const config = this.getByJid(jid);
    if (!config || !config.schedule_enabled) return true;
...
let repositoryInstance: ChatConfigRepository | null = null;

export function getChatConfigRepository(): ChatConfigRepository {
