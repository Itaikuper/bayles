...
import type { ScheduleArgs, CalendarListArgs, CalendarCreateArgs, CalendarUpdateArgs, CalendarDeleteArgs, SendMessageArgs, MediationSession, ChoreRotationArgs } from '../types/index.js';
...
🔑 ID: ${scheduleId}`;
   ...
   */
  private async handleChoreRotation(
    jid: string,
    args: ChoreRotationArgs,
    originalMessage: proto.IWebMessageInfo
  ): Promise<string> {
  ...
  private async resolveScheduleTarget(targetName: string, currentJid: string): Promise<string> {
    ...
    const chatMatch = allChats.find(c =>
      c.display_name?.toLowerCase() === normalizedTarget
    ) || allChats.find(c =>
    ...
    try {
      switch (name) {
        ...
        case 'delete_task': {
          ...
          await this.whatsapp.sendReply(
            jid,
            ok ? `🗑️ נמחקה משימה #${id}: ${existing.title}` : `לא הצלחתי למחוק #${id}.`,
            message,
          );
      ...
      switch (name) {
        case 'gmail_list_recent_emails': {
          const list = await this.gmailService.listRecentEmails(jid, {
            labelName: args.labelName as string | undefined,
        ...
        case 'gmail_read_email': {
          const msg = await this.gmailService.readEmail(jid, args.messageId as string);
        ...
        case 'gmail_add_watch_label': {
          const res = await this.gmailService.addWatchLabel(jid, args.labelName as string);
          if (!res.ok) {
        ...
        case 'gmail_remove_watch_label': {
          const n = await this.gmailService.removeWatchLabel(jid, args.labelName as string);
