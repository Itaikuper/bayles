...
const gmailListRecentDeclaration: FunctionDeclaration = {
...
const gmailReadEmailDeclaration: FunctionDeclaration = {
...
const gmailAddWatchLabelDeclaration: FunctionDeclaration = {
...
const gmailRemoveWatchLabelDeclaration: FunctionDeclaration = {
...
const gmailListWatchLabelsDeclaration: FunctionDeclaration = {
...
const draftNewEmailDeclaration: FunctionDeclaration = {
...
const listCalendarSmartDeclaration: FunctionDeclaration = {
...
const createCalendarSmartDeclaration: FunctionDeclaration = {
...
4. מקסימום תגית אחת בתשובה.`;
   ...
   */
  formatRecentForClassifier(jid: string, tenantId: string = 'default', turns: number = 6): string {
    ...
    return slice
      .map(m => {
        const text = m.parts?.map(p => (p as { text?: string }).text || '').join('').trim();
        if (!text) return '';
...
Messages in [brackets] in conversation history are factual records of actions you performed. Trust them completely — if it says [שלחתי הודעה ל...], you DID send that message. Never deny or contradict these records.`;
      ...
      if (mode === 'owner') {
        ...
        functionDeclarations.push(
          ...
          draftNewEmailDeclaration,
          listCalendarSmartDeclaration,
          createCalendarSmartDeclaration,
...
שאל את המשתמש מה הוא רוצה לעשות עם החומר.`;
  ...
  async parseDateTimePhrase(opts: {
    ...
    todayIso: string; // YYYY-MM-DD in caller's timezone
    timezone: string;
  }): Promise<{
    startIso: string;
    endIso: string;
...
Return JSON: { startIso, endIso, note }. Note is a one-line warning if you had to guess (e.g., "no time given, assumed 09:00"); empty string otherwise.`;
    ...
    const parsed = JSON.parse(response.text?.trim() || '{}') as { startIso?: string; endIso?: string; note?: string };
    return {
