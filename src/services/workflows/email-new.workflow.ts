 ...
 */
export async function runEmailNewWorkflow(
  jid: string,
  slots: EmailNewSlots,
  ...
  if (!email) {
    await whatsapp.sendReply(
      jid,
      `לא מצאתי את "${recipientHint}" בזיכרון. מה הכתובת המלאה?`,
      message,
    );
    return true;
  }
  ...
  let priorThreadSnippets: string[] | undefined;
  try {
  ...
  let ownerCoreMemory: string | undefined;
  try {
  ...
  try {
    ...
    await whatsapp.sendReply(
      jid,
      `✅ טיוטה מוכנה.\n📬 ל: ${label ? `${label} <${email}>` : email}\n📝 ${subject}\n\n${preview}\n\n🔗 ${link}\n_לא נשלח — פתח לבדיקה ושלח מהדפדפן._`,
      message,
    );
