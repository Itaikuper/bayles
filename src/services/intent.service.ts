...
export interface ClassifiedIntent {
  ...
  slots: {
    ...
    topic_hint?: string;
    // calendar
    date_phrase?: string;       // raw Hebrew/English phrase: "היום", "מחר", "השבוע", "ב-15 לחודש", ...
    time_phrase?: string;       // "ב21:30", "10 בבוקר", "מ-3 עד 5", ...
    summary_hint?: string;      // event title/short description
    duration_phrase?: string;   // "שעה", "30 דקות", ...
    calendar_hint?: string;     // "ביומן של אסתר", "ביומן המשותף", "esterkuper", ...
    query_hint?: string;        // free-text filter for list ("פגישות עם דנה", ...)
  };
  reasoning?: string;
}
...
export class IntentService {
   ...
   */
  async classify(userMessage: string, recentHistory?: string): Promise<ClassifiedIntent> {
...
let instance: IntentService | null = null;
