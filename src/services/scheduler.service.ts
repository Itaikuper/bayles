...
export class SchedulerService {
  ...
  scheduleMessage(
    ...
    const task: ScheduledTask = cron.schedule(
      cronExpression,
      this.createExecutionCallback(id, jid, message, oneTime, useAi)
    );
   ...
   */
  scheduleOneTimeMessage(jid: string, message: string, date: Date, useAi: boolean = false): string {
    const now = new Date();
    if (date <= now) {
  ...
  listScheduledMessages(): ScheduledMessageInfo[] {
    return Array.from(this.scheduledMessages.values()).map(
      ({ id, jid, message, cronExpression, oneTime, useAi }) => ({
        id,
        jid,
        message,
        cronExpression,
  ...
  private scheduleMessageWithId(
    ...
    const task: ScheduledTask = cron.schedule(
      cronExpression,
      this.createExecutionCallback(id, jid, message, oneTime, useAi, true)
    );
