...
export function createBotControlRoutes(botControl, whatsapp) {
    const router = Router();
    // ============== Global Settings ==============
    // Get all settings
    ...
    router.post('/chats', async (req, res) => {
        try {
            const { jid, display_name, is_group, enabled, ai_mode, custom_prompt, auto_reply_message } = req.body;
            if (!jid) {
    ...
    router.post('/chats/:jid/toggle', (req, res) => {
        const jid = decodeURIComponent(req.params.jid);
        const { enabled } = req.body;
        if (typeof enabled !== 'boolean') {
            return res.status(400).json({ error: 'enabled (boolean) is required' });
    ...
    router.get('/activity/:jid', (req, res) => {
        const jid = decodeURIComponent(req.params.jid);
