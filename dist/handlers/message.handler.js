...
export class MessageHandler {
    ...
    async runAIWithDispatch(jid, cleanText, isGroup, sender, message, customPrompt) {
        try {
            ...
            if (isOwner) {
                const dispatch = async (name, args) => {
                    try {
                        ...
                        if (name === 'draft_new_email' && this.gmailService) {
                            const a = args;
                            await runEmailNewWorkflow(jid, { recipient_hint: a.recipient_hint, topic_hint: a.topic, subject_hint: a.subject_hint }, message, { gemini: this.gemini, gmail: this.gmailService, whatsapp: this.whatsapp });
                            return { result: { ok: true, note: 'draft created, user notified via WhatsApp' } };
                        }
                        if (name === 'list_calendar_smart' && this.calendarService) {
                            const a = args;
                            await runCalendarListWorkflow(jid, a, message, { gemini: this.gemini, calendar: this.calendarService, whatsapp: this.whatsapp });
                            return { result: { ok: true, note: 'events listed, user notified via WhatsApp' } };
                        }
                        if (name === 'create_calendar_smart' && this.calendarService) {
                            const a = args;
                            await runCalendarCreateWorkflow(jid, a, message, { gemini: this.gemini, calendar: this.calendarService, whatsapp: this.whatsapp });
                        ...
                        if (name === 'list_calendar_events') {
                            const summary = await this.handleCalendarList(jid, args, message);
                            return { result: { summary: summary || 'done' } };
                        }
                        if (name === 'create_calendar_event') {
                            const summary = await this.handleCalendarCreate(jid, args, message);
                            return { result: { summary: summary || 'done' } };
                        }
                        if (name === 'update_calendar_event') {
                            const summary = await this.handleCalendarUpdate(jid, args, message);
                            return { result: { summary: summary || 'done' } };
                        }
                        if (name === 'delete_calendar_event') {
                            const summary = await this.handleCalendarDelete(jid, args, message);
                            return { result: { summary: summary || 'done' } };
                        }
                        // Owner-available misc tools.
                        if (name === 'create_schedule') {
                            const summary = await this.handleScheduleFunctionCall(jid, args, message);
                            return { result: { summary: summary || 'done' } };
                        }
                ...
                const response = await this.gemini.generateResponse(jid, messageForAI, customPrompt, 'default', sender || undefined, intentHint, dispatch);
     ...
     */
    async extractQuotedContent(message) {
        ...
        if (qm.audioMessage) {
            try {
                ...
                if (!trimmed || /^לא זוהה דיבור\.?$/.test(trimmed))
                    return null;
        ...
        if (t?.trim())
            return { kind: 'text', content: t.trim() };
        const c = qm.imageMessage?.caption?.trim();
        if (c)
            return { kind: 'image', content: c };
        return null;
    }
...
/birthdays add איתי 5 פבר יהודה 25 מרץ`;
     ...
     */
    async handleTaskFunctionStructured(jid, name, args) {
        if (!this.gmailService || !this.gmailService.isOwner(jid)) {
            ...
            switch (name) {
                ...
                case 'edit_task': {
                    ...
                    if (!id && args.query) {
                        const matches = repo.findByTitle(jid, String(args.query));
                        if (matches.length === 0)
                            return { error: `no task matching "${args.query}"` };
                        if (matches.length > 1) {
                    ...
                    if (!id)
                        return { error: 'no id or query provided' };
                    const existing = repo.getById(id);
                    if (!existing || existing.jid !== jid)
                    ...
                    const updated = repo.edit(id, patch);
                    if (!updated)
                        return { error: `failed to update #${id}` };
                    return {
                        ok: true,
                        id: updated.id,
                        title: updated.title,
